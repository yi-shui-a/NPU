...
public class FushiSystem {
    ...
    private void lookupTestScore() throws IOException {
        ...
        if(studentCatalog.getStudent(id)==null){
            stdErr.println("Input error, please re-enter");
            lookupTestScore();
            return;
        }
        if(studentCatalog.getStudent(id).getExamPaper()==null) {
            stdErr.println("The student hasn't got a test paper yet. Please complete it.");
            return;
        }
        for(int i=0;i<studentCatalog.getStudent(id).getExamPaper().getNumberOfTestItems();i++){
            System.out.println("The score of item "+ (i+1) + " is:"+
                    studentCatalog.getStudent(id).getExamPaper().getTestItem(i).getScore());
        }
    }
    ...
    private void lookupTotalScore() throws IOException {
        ...
        Scanner scanner = new Scanner(System.in);
        String id=scanner.nextLine();
        for(Student n: studentCatalog.getStudents()){
            if(id.equals(n.getId())){
                if(n.getExamPaper()!=null){
                    if(n.getExamPaper().getTotalScore()==0){
                        System.out.println("The student hasn't finished the test paper.");
                        return;
                    }
                    System.out.println("The total score of the student is: " +n.getExamPaper().getTotalScore());
                    return;
                }else {
                    stdErr.println("The student hasn't got a test paper yet. Please complete it.");
                    return;
        ...
        for (Student student : this.studentCatalog.getStudents()) {
            if (student.getId().equals(id)) {
                ...
                if (student.getExamPaper() == null) {
                    ...
                    for (int i = 0; i < examPaper.getNumberOfTestItems(); i++) {
    ...
    private void displayExamPaper() throws IOException {
        ...
        if (student.getExamPaper() == null) {
            ...
            for (int n = 0; n < 10; n++) {
                System.out.println(student.getExamPaper().getTestItem(n).getTest().getCode() + " | " +
                        student.getExamPaper().getTestItem(n).getTest().getTitle() + " | " +
                        student.getExamPaper().getTestItem(n).getTest().getDifficultyDegree());
            }
    }
